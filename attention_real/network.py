...
class TypeAttention:
	...
	def __init__(self, num_steps, num_steps_mention, num_steps_label, hidden_size, num_type_dim, num_word_dim, num_mention_dim, num_types, num_words, num_mention_words, learning_rate, batch_size, max_gradient_norm, attention_type, output_activation, descs):
	...
	def type_context_matmul(self, sentence_states):
		...
		#sentence_type_context_W_type_embedding is a list of #num_steps tensors of dimension (#batch_size * #num_types)
		return sentence_type_context_W_type_embedding
	
	def attn_on_types_given_context(self, sentence_states, num_heads=1):
		...
		with tf.variable_scope(self.type_context_scope) as scope:
			...
			#sentence_type_attn is a list of #num_steps tensors of dimension (#batch_size * #num_types)
			sentence_type_attn = tf.concat(0, sentence_type_attn)
			#sentence_type_attn is a tensor of dimension (#num_steps * #batch_size * #num_types)
			sentence_type_attn = tf.nn.softmax(sentence_type_attn, dim=0)
			sentence_type_attn = tf.transpose(sentence_type_attn)
			#sentence_type_attn is a tensor of dimension (#num_types * #batch_size * #num_steps)
			sentence_type_attn = tf.expand_dims(sentence_type_attn, axis=3)
			#sentence_type_attn is a tensor of dimension (#num_types * #batch_size * #num_steps * 1)
			...
			#sentence_states_replicated is a tensor of dimension (#num_types * #batch_size * #num_steps * hidden_size)
	...
	def type_mention_matmul(self, sentence_mention_states):
                ...
                #sentence_type_mention_W_type_embedding is a list of #num_steps_mention tensors of dimension (#batch_size * #num_types)
                return sentence_type_mention_W_type_embedding

        def attn_on_types_given_mention(self, sentence_mention_states, num_heads=1):
                ...
                with tf.variable_scope(self.type_mention_scope) as scope:
                        ...
                        #sentence_type_attn is a list of #num_states_mention tensors of dimension (#batch_size * #num_types)
                        sentence_type_attn = tf.concat(0, sentence_type_attn)
                        #sentence_type_attn is a tensor of dimension (#num_states_mention * #batch_size * #num_types)
                        sentence_type_attn = tf.nn.softmax(sentence_type_attn, dim=0)
                        sentence_type_attn = tf.transpose(sentence_type_attn)
                        #sentence_type_attn is a tensor of dimension (#num_types * #batch_size * #num_steps_mention)
                        sentence_type_attn = tf.expand_dims(sentence_type_attn, axis=3)
                        #sentence_type_attn is a tensor of dimension (#num_types * #batch_size * #num_steps_mention * 1)
                        ...
                        sentence_states_replicated = tf.tile(sentence_states_replicated, [self.num_types,1,1,1])
                        #sentence_states_replicated is a tensor of dimension (#num_types * #batch_size * #num_steps_mention * hidden_size)
                        attn = tf.multiply(sentence_type_attn,  sentence_states_replicated)
                        attn = tf.reduce_sum(attn, 2)
                        #attn is a tensor of dimension (#num_types * #batch_size * #hidden_size)
                        attn = tf.transpose(attn, perm=[1,0,2])
	...
	def decode(self, concatenated_input, loop_fn, init_state, sentence_states, sentence_outputs, sentence_mention_states, sentence_mention_outputs, dec_scope, attn_len, attn_size, attn_t_given_context, attn_t_given_mention, initial_state_attention=False):
		state = init_state
		outputs = []
		prev = None
		output_size = self.dec_cells.output_size
		def attention(query, attn_t_given_context, attn_t_given_mention):
			with variable_scope.variable_scope("Attention"):
				y = tf.tile(array_ops.reshape(query, [-1, 1, self.hidden_size]), [1, self.num_types, 1])
				#y is of dimension batch_size * num_types * hidden_size
				#attn_t_given_context is of dimension batch_size * num_types * hidden_size
				#attn_t_given_mention is of dimension batch_size * num_types * hidden_size
				#type_embedding is of dimension num_types * num_type_dim
				...
				#attn_t_given_context is of dimension batch_size * num_types * hidden_size
                                #attn_t_given_mention is of dimension batch_size * num_types * hidden_size
				type_context_attn = tf.matmul(self.type_embedding, self.type_context_attnW, transpose_b=True)
				type_mention_attn = tf.matmul(self.type_embedding, self.type_mention_attnW, transpose_b=True)
				...
				#type_context_attn is of dimension batch_size * num_types * hidden_size
				#type_mention_attn is of dimension batch_size * num_types * hidden_size
				...
				#attn_t_given_context is of dimension batch_size * num_types
				#attn_t_given_mention is of dimension batch_size * num_types
				attn_t = attn_t_given_context + attn_t_given_mention
				attn_t = nn_ops.softmax(attn_t)
				if self.attention_type==self.CHANGE_OF_VARIABLES:
					attn_t = self.attention_aggregation(attn_t)
				#attn_t is of dimension batch_size * num_types
				hidden_t = tf.matmul(attn_t, self.type_embedding)
		...
		for i,inp in enumerate(concatenated_input):
			...
			if i==0 and initial_state_attention:	
				with tf.variable_scope(dec_scope, reuse=True):
					attns = attention(state, attn_t_given_context, attn_t_given_mention)
			else:
				attns = attention(state, attn_t_given_context, attn_t_given_mention)
	...
	def decoder(self, sentence_states, sentence_outputs, sentence_mention_states, sentence_mention_outputs, initial_state_attention=False):
		...
		with tf.variable_scope(self.dec_scope) as scope:
			...
			attn_t_given_context = self.attn_on_types_given_context(sentence_outputs)
			attn_t_given_mention = self.attn_on_types_given_mention(sentence_mention_outputs)
			def feed_previous_decode(feed_previous_bool):
				...
				#concatenated_inputs is a list of self.max_steps_label tensors of dimension batch_size * (self.num_type_dim + self.hidden_size)				
				dec_output, _ = self.decode(concatenated_input, loop_fn, init_state, sentence_states, sentence_outputs, sentence_mention_states, sentence_mention_outputs, scope, attn_len, attn_size, attn_t_given_context, attn_t_given_mention, initial_state_attention)
				return dec_output
